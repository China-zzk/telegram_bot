...
GITHUB_REPO = os.environ.get("GITHUB_REPO")  # 使用自定义环境变量
...
async def run_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # 调用 GitHub API 触发工作流
        url = f"https://api.github.com/repos/{GITHUB_REPO}/actions/workflows/telegram_bot.yml/dispatches"
        headers = {
...
def run_command_bot():
    """启动命令机器人"""
    # 检查必要的环境变量
    if not all([COMMAND_BOT_TOKEN, GITHUB_TOKEN, GITHUB_REPO]):
        logger.error("请设置 COMMAND_BOT_TOKEN, GITHUB_TOKEN 和 GITHUB_REPO 环境变量")
        exit(1)
